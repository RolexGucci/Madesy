"""
Slayd (PowerPoint) tayyorlovchi modul.

Oqim:
  1. Gemini API - mavzu + slaydlar soniga qarab har bir slayd uchun
     sarlavha/matn/rasm kalit so'zini generatsiya qiladi (matn, bepul reja).
  2. Pexels API - har bir slayd uchun kalit so'z asosida mos fotosurat topadi
     (tayyor stok fotosurat, bepul reja).
  3. python-pptx - tanlangan dizayn shabloniga (4 tadan biriga) qarab hammasini
     PPTX faylga yig'adi.

Og'ir mahalliy AI model yo'q - faqat ikkita tashqi API'ga qisqa so'rov,
shuning uchun server uchun yengil.
"""
import io
import json
import logging
import os
import time

import requests
from pptx import Presentation
from pptx.dml.color import RGBColor
from pptx.enum.shapes import MSO_SHAPE
from pptx.enum.text import PP_ALIGN
from pptx.util import Inches, Pt

logger = logging.getLogger(__name__)

GEMINI_API_KEY = os.getenv("GEMINI_API_KEY", "")
PEXELS_API_KEY = os.getenv("PEXELS_API_KEY", "")

# Matn generatsiyasi uchun aniq model nomlari ishlatiladi (alias emas -
# "gemini-flash-latest" kabi aliaslar Google tomonidan istalgan vaqtda
# eng yangi, ko'pincha hali tirband/beqaror preview modelga qayta
# yo'naltirilishi mumkin, bu bizning nazoratimizdan tashqarida).
#
# Zanjir tartibi: birinchi model bilan urinib ko'ramiz, agar u "high
# demand" (503) xatosi qaytarsa, ro'yxatdagi keyingisiga o'tamiz.
# Barchasi Flash-Lite darajasida - matn/JSON generatsiyasi uchun bu
# yetarli va eng tejamkor, chuqur reasoning kerak emas.
GEMINI_MODEL_CHAIN = [
    "gemini-3.1-flash-lite",
    "gemini-3.5-flash-lite",
    "gemini-2.5-flash-lite",
]
GEMINI_URL_TEMPLATE = "https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent"
PEXELS_SEARCH_URL = "https://api.pexels.com/v1/search"


def _call_gemini_with_fallback(payload: dict, timeout: int):
    """GEMINI_MODEL_CHAIN ro'yxatidagi modellarni ketma-ket sinab ko'radi.
    Agar model "high demand" (503) xatosi qaytarsa - keyingi modelga
    o'tadi (bir marta qisqa kutib). Agar boshqa turdagi xato (masalan
    400/401) bo'lsa yoki muvaffaqiyatli (200) bo'lsa - darhol qaytaradi,
    zanjirni davom ettirmaydi."""
    resp = None
    for i, model in enumerate(GEMINI_MODEL_CHAIN):
        url = GEMINI_URL_TEMPLATE.format(model=model)
        resp = requests.post(f"{url}?key={GEMINI_API_KEY}", json=payload, timeout=timeout)
        if resp.status_code != 503:
            return resp
        logger.warning(f"Gemini modeli band ({model}, 503) - keyingi modelga o'tilmoqda")
        if i < len(GEMINI_MODEL_CHAIN) - 1:
            time.sleep(1.5)
    return resp

MIN_SLIDES = 5
MAX_SLIDES = 30
PRICE_PER_SLIDE = 500

TEMPLATES = {
    "blue": "Ko'k klassik",
    "dark": "Tungi tex",
    "warm": "Iliq ijodiy",
    "yellow": "Sariq hisobot",
}


# ─────────────────────────────────────────────────────────
# 1-qadam: Gemini - slaydlar tarkibini generatsiya qilish
# ─────────────────────────────────────────────────────────

def generate_outline(topic: str, num_slides: int) -> list:
    """Gemini API orqali mavzuga mos slaydlar tarkibini (JSON) generatsiya qiladi."""
    if not GEMINI_API_KEY:
        raise RuntimeError("GEMINI_API_KEY sozlanmagan. Render'da Environment bo'limiga qo'shing.")

    prompt = f"""Sen professional taqdimot (prezentatsiya) tayyorlovchi yordamchisan.
Mavzu: "{topic}"
Jami slaydlar soni: {num_slides}

FAQAT quyidagi JSON massivini qaytar (boshqa hech qanday matn, izoh yoki ``` belgisi yozma):

[
  {{"type": "title", "title": "...", "subtitle": "..."}},
  {{"type": "content", "title": "...", "bullets": ["...", "...", "..."], "image_keyword": "..."}},
  {{"type": "cards_3", "title": "...", "cards": [{{"title": "...", "desc": "..."}}, {{"title": "...", "desc": "..."}}, {{"title": "...", "desc": "..."}}]}},
  {{"type": "stat_highlight", "title": "...", "stat": "80%", "stat_label": "...", "desc": "..."}},
  {{"type": "split_image_text", "title": "...", "text": "...", "image_keyword": "..."}},
  {{"type": "divider", "title": "Xulosa", "subtitle": "..."}}
]

Qolgan slaydlar uchun {{"type": ...}} tanlashda qoidalar - har bir slayd mazmuniga ENG MOS turni tanla:
- "content": umumiy tushuntirish, 5-6 ta bullet fikr + rasm kerak bo'lsa. Har doim "image_keyword" maydoni qo'sh.
- "cards_3": mazmun aniq 3 ta alohida, teng darajadagi fikr/bosqich/afzallikdan iborat bo'lsa (masalan "3 ta asosiy xususiyat", "3 bosqich"). "cards" massivida ROPPA-ROSA 3 ta element, har birida qisqa "title" (2-4 so'z) va "desc" (8-14 so'z). Rasm kerak emas.
- "stat_highlight": mazmunda BITTA aniq raqam/foiz/statistika markaziy o'rin egallasa (masalan "80% talabalar...", "3 baravar oshdi"). "stat" - juda qisqa raqam/foiz matni (masalan "80%", "3x", "24/7"), "stat_label" - shu raqamning qisqa izohi (3-6 so'z), "desc" - qo'shimcha 1 gap tushuntirish. Rasm kerak emas.
- "split_image_text": mazmun HIKOYA/HISSIY/TASVIRIY uslubda bo'lsa (masalan tarixiy voqea, sayohat, shaxsiy tajriba, muhim lahza tasviri) - bullet ro'yxat emas, balki 2-3 gapdan iborat, ravon yozilgan "text" maydoni (30-50 so'z) + katta "image_keyword". Rasm bu yerda katalog ko'rinishida emas, hikoyaga hamroh sifatida ishlatiladi.
- Slaydlar turini xilma-xil qilib tanla - hammasi bir xil turda bo'lmasin, taqdimot davomida "content", "cards_3", "stat_highlight", "split_image_text" aralash kelsin (lekin faqat mazmunga chinakam mos bo'lgandagina, zo'rma-zo'raki emas)

Qat'iy qoidalar:
- Jami roppa-rosa {num_slides} ta element (slayd) bo'lsin
- 1-slayd har doim "title" turida (mavzu sarlavhasi + qisqa subtitle)
- Oxirgi slayd "divider" turida ("Xulosa" yoki "Yakun" kabi)
- Qolgan barcha slaydlar yuqoridagi qoidaga ko'ra "content", "cards_3", "stat_highlight" yoki "split_image_text" turlaridan biri
- Matn (title, subtitle, bullets, cards, stat_label, desc, text) - o'zbek tilida
- image_keyword "content" va "split_image_text" turlarida kerak - Pexels FAQAT real-hayotiy stok-fotosuratlar (odamlar, joylar, buyumlar, tabiat, harakatlar) bazasi, mavhum yoki ilmiy-texnik atamalarni TUSHUNMAYDI. Shuning uchun image_keyword mavzuning o'zini emas, balki o'sha mavzuni ESLATADIGAN, KAMERADA TUSHIRISH MUMKIN bo'lgan konkret vizual sahnani tasvirlashi kerak:
  - FAQAT ingliz tilida, 2-4 so'z
  - Har doim konkret ot/sahna: odam(lar), joy, buyum, harakat, tabiat manzarasi
  - Mavhum, ilmiy yoki tushunchaviy so'zlardan qochish (masalan "photosynthesis", "economic growth", "quantum theory" - bularni Pexels tushunmaydi)
  - Buning o'rniga o'sha g'oyani chaqiradigan real sahna tanlash (masalan "photosynthesis" o'rniga "green leaf sunlight macro"; "economic growth" o'rniga "city skyline business district"; "quantum theory" o'rniga "physics laboratory scientist")
  - Yomon misollar: "artificial intelligence", "success", "teamwork spirit", "innovation"
  - Yaxshi misollar: "robot hand technology", "people shaking hands office", "students studying library", "doctor patient hospital", "mountain sunrise landscape"
- Bullets qisqa, aniq, taqdimotga mos uslubda bo'lsin, gap oxirida nuqta qo'yma
"""

    payload = {
        "contents": [{"parts": [{"text": prompt}]}],
        "generationConfig": {"temperature": 0.7, "maxOutputTokens": 32768},
    }

    resp = _call_gemini_with_fallback(payload, timeout=90)
    if resp.status_code != 200:
        raise RuntimeError(f"Gemini xatosi ({resp.status_code}): {resp.text[:300]}")

    data = resp.json()
    try:
        text = data["candidates"][0]["content"]["parts"][0]["text"].strip()
    except (KeyError, IndexError) as e:
        raise RuntimeError(f"Gemini javobini o'qib bo'lmadi: {data}") from e

    # Model ba'zan ```json bilan o'raydi - tozalaymiz
    if text.startswith("```"):
        text = text.strip("`")
        if text.startswith("json"):
            text = text[4:]

    try:
        slides = json.loads(text.strip())
    except json.JSONDecodeError as e:
        raise RuntimeError(f"Gemini JSON qaytarmadi: {text[:300]}") from e

    if not isinstance(slides, list) or not slides:
        raise RuntimeError("Gemini bo'sh yoki noto'g'ri natija qaytardi")

    return slides


# ─────────────────────────────────────────────────────────
# 2-qadam: Pexels - mavzuga mos fotosurat topish
# ─────────────────────────────────────────────────────────

def _pexels_search_once(query: str) -> bytes:
    """Bitta so'rov bilan Pexels'dan rasm qidiradi. Topilmasa None."""
    resp = requests.get(
        PEXELS_SEARCH_URL,
        params={"query": query, "per_page": 1},
        headers={"Authorization": PEXELS_API_KEY},
        timeout=20,
    )
    if resp.status_code != 200:
        logger.warning(f"Pexels xatosi ({resp.status_code}) '{query}' uchun")
        return None
    photos = resp.json().get("photos", [])
    if not photos:
        return None
    img_url = photos[0]["src"]["large"]
    img_resp = requests.get(img_url, timeout=20)
    img_resp.raise_for_status()
    return img_resp.content


def fetch_pexels_image(keyword: str) -> bytes:
    """Pexels'dan kalit so'zga mos fotosuratni yuklab oladi.
    Agar to'liq kalit so'z (masalan 3-4 so'zli) natija bermasa, so'zlarni
    qisqartirib (oxirgi so'zni tashlab) qayta so'raladi - chunki Pexels
    ko'pincha uzun/spetsifik so'rovlarda kam natija qaytaradi, lekin
    qisqaroq umumiyroq so'rovda mos rasm topiladi. Baribir topilmasa None."""
    if not PEXELS_API_KEY or not keyword:
        return None
    words = keyword.split()
    try:
        # 1-urinish: to'liq kalit so'z
        img = _pexels_search_once(keyword)
        if img:
            return img
        # 2-urinish: oxirgi so'zni tashlab, qisqaroq va umumiyroq qidiruv
        if len(words) > 1:
            shorter = " ".join(words[:-1])
            img = _pexels_search_once(shorter)
            if img:
                return img
        # 3-urinish: faqat birinchi so'z (eng umumiy)
        if len(words) > 2:
            img = _pexels_search_once(words[0])
            if img:
                return img
        return None
    except Exception:
        logger.exception(f"Pexels'dan rasm olishda xato: {keyword}")
        return None


# ─────────────────────────────────────────────────────────
# 3-qadam: python-pptx - dizayn shablonlari
# ─────────────────────────────────────────────────────────

def _set_no_line(shape):
    shape.line.fill.background()


def _add_text(slide, x, y, w, h, text, size, color, bold=False, align=PP_ALIGN.LEFT, font="Arial"):
    tb = slide.shapes.add_textbox(x, y, w, h)
    tf = tb.text_frame
    tf.word_wrap = True
    tf.margin_left = 0
    tf.margin_right = 0
    tf.margin_top = 0
    tf.margin_bottom = 0
    p = tf.paragraphs[0]
    p.alignment = align
    run = p.add_run()
    run.text = text
    run.font.size = Pt(size)
    run.font.color.rgb = color
    run.font.bold = bold
    run.font.name = font
    return tb


def _add_bullets(slide, x, y, w, h, items, size, color, font="Arial"):
    tb = slide.shapes.add_textbox(x, y, w, h)
    tf = tb.text_frame
    tf.word_wrap = True
    tf.margin_left = 0
    tf.margin_right = 0
    for i, item in enumerate(items):
        p = tf.paragraphs[0] if i == 0 else tf.add_paragraph()
        p.space_after = Pt(10)
        run = p.add_run()
        run.text = "•  " + item
        run.font.size = Pt(size)
        run.font.color.rgb = color
        run.font.name = font
    return tb


def _bg_fill(prs, slide, color):
    bg = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, 0, 0, prs.slide_width, prs.slide_height)
    bg.fill.solid()
    bg.fill.fore_color.rgb = color
    _set_no_line(bg)
    return bg


def _add_image_or_fallback(slide, image_bytes, x, y, w, h, fallback_color):
    """Rasm bo'lsa joylaydi (to'ldirib kesib), bo'lmasa rangli to'rtburchak qo'yadi."""
    if image_bytes:
        try:
            slide.shapes.add_picture(io.BytesIO(image_bytes), x, y, width=w, height=h)
            return
        except Exception:
            logger.exception("Rasmni pptx'ga joylashda xato")
    box = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, x, y, w, h)
    box.fill.solid()
    box.fill.fore_color.rgb = fallback_color
    _set_no_line(box)


# ---------- Umumiy: cards_3 va stat_highlight (barcha shablonlarda bir xil mantiq, faqat rang farqlanadi) ----------

def _draw_cards_3(prs, s, sd, accent, card_bg, title_color, desc_color, text_on_accent):
    """3 ustunli karta layout - har bir kartada raqam-belgi, sarlavha va tavsif."""
    _add_text(s, Inches(0.9), Inches(0.7), Inches(11.5), Inches(1.0), sd.get("title", ""), 30, title_color, bold=True)
    cards = sd.get("cards", [])[:3]
    card_w = Inches(3.7)
    gap = Inches(0.35)
    total_w = card_w * 3 + gap * 2
    start_x = int((prs.slide_width - total_w) / 2)
    y = Inches(2.4)
    card_h = Inches(4.0)
    for i, card in enumerate(cards):
        x = start_x + i * (card_w + gap)
        box = s.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, x, y, card_w, card_h)
        box.adjustments[0] = 0.06
        box.fill.solid(); box.fill.fore_color.rgb = card_bg; _set_no_line(box)
        badge = s.shapes.add_shape(MSO_SHAPE.OVAL, x + Inches(0.35), y + Inches(0.35), Inches(0.7), Inches(0.7))
        badge.fill.solid(); badge.fill.fore_color.rgb = accent; _set_no_line(badge)
        _add_text(s, x + Inches(0.35), y + Inches(0.42), Inches(0.7), Inches(0.5), str(i + 1), 20, text_on_accent, bold=True, align=PP_ALIGN.CENTER)
        _add_text(s, x + Inches(0.35), y + Inches(1.25), card_w - Inches(0.7), Inches(0.7), card.get("title", ""), 18, title_color, bold=True)
        _add_text(s, x + Inches(0.35), y + Inches(2.0), card_w - Inches(0.7), Inches(1.7), card.get("desc", ""), 13, desc_color)


def _draw_stat_highlight(prs, s, sd, accent, title_color, desc_color):
    """Bitta katta statistika markazda - raqam ulkan shriftda, atrofida qisqa izoh."""
    _add_text(s, Inches(0.9), Inches(0.7), Inches(11.5), Inches(1.0), sd.get("title", ""), 28, title_color, bold=True)
    _add_text(s, Inches(0.9), Inches(2.3), Inches(11.5), Inches(2.3), sd.get("stat", ""), 96, accent, bold=True, align=PP_ALIGN.CENTER)
    _add_text(s, Inches(1.5), Inches(4.7), Inches(10.3), Inches(0.7), sd.get("stat_label", ""), 22, title_color, bold=True, align=PP_ALIGN.CENTER)
    _add_text(s, Inches(2.2), Inches(5.5), Inches(9), Inches(1.0), sd.get("desc", ""), 15, desc_color, align=PP_ALIGN.CENTER)


def _draw_split_image_text(prs, s, sd, title_color, text_color, placeholder_color, image_right=True):
    """Katta rasm (yarim ekran) + hikoya uslubidagi qisqa matn (bullet emas, ravon gap)."""
    img_w = Inches(6.4)
    if image_right:
        img_x = prs.slide_width - img_w
        text_x = Inches(0.9)
    else:
        img_x = 0
        text_x = img_w + Inches(0.5)
    _add_image_or_fallback(s, sd.get("_image"), img_x, 0, img_w, prs.slide_height, placeholder_color)
    text_w = prs.slide_width - img_w - Inches(1.4)
    _add_text(s, text_x, Inches(1.4), text_w, Inches(1.4), sd.get("title", ""), 30, title_color, bold=True)
    _add_text(s, text_x, Inches(3.0), text_w, Inches(3.5), sd.get("text", ""), 17, text_color)


# ---------- Shablon 1: Ko'k klassik ----------

def _render_blue(prs, slides):
    BLUE = RGBColor(0x1B, 0x4F, 0x72)
    WHITE = RGBColor(0xFF, 0xFF, 0xFF)
    DARK = RGBColor(0x1A, 0x1A, 0x1A)
    GRAY = RGBColor(0x5A, 0x5A, 0x5A)
    PLACEHOLDER = RGBColor(0xD9, 0xE2, 0xEA)
    blank = prs.slide_layouts[6]

    for i, sd in enumerate(slides):
        s = prs.slides.add_slide(blank)
        _bg_fill(prs, s, WHITE)
        t = sd.get("type", "content")

        if t == "title":
            blk = s.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(-2), Inches(5.8), Inches(9), Inches(5))
            blk.rotation = -8
            blk.fill.solid(); blk.fill.fore_color.rgb = BLUE; _set_no_line(blk)
            _add_text(s, Inches(0.9), Inches(2.3), Inches(9.8), Inches(1.8), sd.get("title", ""), 38, DARK, bold=True)
            _add_text(s, Inches(0.9), Inches(4.3), Inches(9.5), Inches(0.6), sd.get("subtitle", ""), 18, GRAY)
            _add_text(s, Inches(0.9), Inches(7.0), Inches(6), Inches(0.5), "MADESY", 13, WHITE, bold=True)

        elif t == "divider":
            _bg_fill(prs, s, BLUE)
            _add_text(s, Inches(1), Inches(2.7), Inches(11.3), Inches(1.6), sd.get("title", ""), 40, WHITE, bold=True)
            _add_text(s, Inches(1), Inches(4.5), Inches(11), Inches(0.6), sd.get("subtitle", ""), 18, RGBColor(0xC9, 0xDD, 0xEA))

        elif t == "cards_3":
            _draw_cards_3(prs, s, sd, accent=BLUE, card_bg=RGBColor(0xF0, 0xF4, 0xF8), title_color=DARK, desc_color=GRAY, text_on_accent=WHITE)

        elif t == "stat_highlight":
            _draw_stat_highlight(prs, s, sd, accent=BLUE, title_color=DARK, desc_color=GRAY)

        elif t == "split_image_text":
            _bg_fill(prs, s, WHITE)
            _draw_split_image_text(prs, s, sd, title_color=DARK, text_color=GRAY, placeholder_color=PLACEHOLDER, image_right=(i % 2 == 0))

        else:
            band = s.shapes.add_shape(MSO_SHAPE.RECTANGLE, 0, 0, Inches(4.6), prs.slide_height)
            band.fill.solid(); band.fill.fore_color.rgb = BLUE; _set_no_line(band)
            _add_image_or_fallback(s, sd.get("_image"), Inches(0.7), Inches(2.1), Inches(3.2), Inches(3.2), PLACEHOLDER)
            _add_text(s, Inches(5.1), Inches(0.8), Inches(7.4), Inches(1.0), sd.get("title", ""), 28, DARK, bold=True)
            _add_bullets(s, Inches(5.1), Inches(2.0), Inches(7.4), Inches(4.5), sd.get("bullets", []), 16, DARK)


# ---------- Shablon 2: Tungi tex ----------

def _render_dark(prs, slides):
    NAVY = RGBColor(0x12, 0x0E, 0x2B)
    PURPLE = RGBColor(0x4A, 0x2F, 0x7A)
    ACCENT = RGBColor(0x8B, 0x5C, 0xF6)
    WHITE = RGBColor(0xFF, 0xFF, 0xFF)
    LIGHT = RGBColor(0xC9, 0xB8, 0xE8)
    PLACEHOLDER = RGBColor(0x2A, 0x22, 0x4A)
    blank = prs.slide_layouts[6]

    for i, sd in enumerate(slides):
        s = prs.slides.add_slide(blank)
        t = sd.get("type", "content")

        if t == "title":
            _bg_fill(prs, s, NAVY)
            glow = s.shapes.add_shape(MSO_SHAPE.OVAL, Inches(9.5), Inches(-1.5), Inches(5), Inches(5))
            glow.fill.solid(); glow.fill.fore_color.rgb = PURPLE; _set_no_line(glow)
            _add_text(s, Inches(0.9), Inches(2.2), Inches(10.5), Inches(1.8), sd.get("title", ""), 36, WHITE, bold=True)
            _add_text(s, Inches(0.9), Inches(4.2), Inches(9), Inches(0.6), sd.get("subtitle", ""), 18, LIGHT)
            _add_text(s, Inches(0.9), Inches(6.5), Inches(6), Inches(0.5), "MADESY", 13, ACCENT, bold=True)

        elif t == "divider":
            _bg_fill(prs, s, PURPLE)
            _add_text(s, Inches(1), Inches(2.7), Inches(11.3), Inches(1.6), sd.get("title", ""), 40, WHITE, bold=True)
            _add_text(s, Inches(1), Inches(4.5), Inches(11), Inches(0.6), sd.get("subtitle", ""), 18, LIGHT)

        elif t == "cards_3":
            _bg_fill(prs, s, NAVY)
            _draw_cards_3(prs, s, sd, accent=ACCENT, card_bg=RGBColor(0x1E, 0x18, 0x3D), title_color=WHITE, desc_color=LIGHT, text_on_accent=WHITE)

        elif t == "stat_highlight":
            _bg_fill(prs, s, NAVY)
            _draw_stat_highlight(prs, s, sd, accent=ACCENT, title_color=WHITE, desc_color=LIGHT)

        elif t == "split_image_text":
            _bg_fill(prs, s, NAVY)
            _draw_split_image_text(prs, s, sd, title_color=WHITE, text_color=LIGHT, placeholder_color=PLACEHOLDER, image_right=(i % 2 == 0))

        else:
            _bg_fill(prs, s, NAVY)
            _add_image_or_fallback(s, sd.get("_image"), Inches(8.3), Inches(1.2), Inches(4.1), Inches(5.1), PLACEHOLDER)
            _add_text(s, Inches(0.9), Inches(0.9), Inches(7), Inches(1.0), sd.get("title", ""), 26, WHITE, bold=True)
            _add_bullets(s, Inches(0.9), Inches(2.1), Inches(7), Inches(4.5), sd.get("bullets", []), 16, LIGHT)


# ---------- Shablon 3: Iliq ijodiy ----------

def _render_warm(prs, slides):
    CORAL = RGBColor(0xE8, 0x7A, 0x5D)
    TEAL = RGBColor(0x3D, 0x8B, 0x8A)
    CREAM = RGBColor(0xFF, 0xFF, 0xFF)
    DARK = RGBColor(0x2B, 0x2B, 0x2B)
    GRAY = RGBColor(0x6B, 0x6B, 0x6B)
    PLACEHOLDER = RGBColor(0xF3, 0xDA, 0xD0)
    blank = prs.slide_layouts[6]

    content_num = 0
    for i, sd in enumerate(slides):
        s = prs.slides.add_slide(blank)
        t = sd.get("type", "content")

        if t == "title":
            _bg_fill(prs, s, CREAM)
            c1 = s.shapes.add_shape(MSO_SHAPE.OVAL, Inches(10.8), Inches(-1.2), Inches(4), Inches(4))
            c1.fill.solid(); c1.fill.fore_color.rgb = CORAL; _set_no_line(c1)
            c2 = s.shapes.add_shape(MSO_SHAPE.OVAL, Inches(-1.5), Inches(5.5), Inches(4), Inches(4))
            c2.fill.solid(); c2.fill.fore_color.rgb = TEAL; _set_no_line(c2)
            _add_text(s, Inches(0.9), Inches(2.5), Inches(10.5), Inches(1.8), sd.get("title", ""), 34, DARK, bold=True)
            _add_text(s, Inches(0.9), Inches(4.4), Inches(9), Inches(0.6), sd.get("subtitle", ""), 18, GRAY)

        elif t == "divider":
            _bg_fill(prs, s, CORAL)
            _add_text(s, Inches(1), Inches(2.7), Inches(11.3), Inches(1.6), sd.get("title", ""), 40, CREAM, bold=True)
            _add_text(s, Inches(1), Inches(4.5), Inches(11), Inches(0.6), sd.get("subtitle", ""), 18, RGBColor(0xFF, 0xE8, 0xDF))

        elif t == "cards_3":
            _draw_cards_3(prs, s, sd, accent=TEAL, card_bg=RGBColor(0xFB, 0xF3, 0xEF), title_color=DARK, desc_color=GRAY, text_on_accent=CREAM)

        elif t == "stat_highlight":
            _draw_stat_highlight(prs, s, sd, accent=CORAL, title_color=DARK, desc_color=GRAY)

        elif t == "split_image_text":
            _bg_fill(prs, s, CREAM)
            _draw_split_image_text(prs, s, sd, title_color=DARK, text_color=GRAY, placeholder_color=PLACEHOLDER, image_right=(i % 2 == 0))

        else:
            content_num += 1
            _bg_fill(prs, s, CREAM)
            badge = s.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.9), Inches(0.7), Inches(1.3), Inches(0.5))
            badge.fill.solid(); badge.fill.fore_color.rgb = TEAL if content_num % 2 else CORAL; _set_no_line(badge)
            _add_text(s, Inches(0.9), Inches(0.82), Inches(1.3), Inches(0.4), f"{content_num:02d}", 15, CREAM, bold=True, align=PP_ALIGN.CENTER)
            _add_image_or_fallback(s, sd.get("_image"), Inches(8.6), Inches(1.3), Inches(3.8), Inches(3.8), PLACEHOLDER)
            _add_text(s, Inches(0.9), Inches(1.55), Inches(7), Inches(1.0), sd.get("title", ""), 26, DARK, bold=True)
            _add_bullets(s, Inches(0.9), Inches(2.7), Inches(7.3), Inches(4.0), sd.get("bullets", []), 15, DARK)


# ---------- Shablon 4: Sariq hisobot ----------

def _render_yellow(prs, slides):
    YELLOW = RGBColor(0xF5, 0xB7, 0x1E)
    BLACK = RGBColor(0x1A, 0x1A, 0x1A)
    WHITE = RGBColor(0xFF, 0xFF, 0xFF)
    GRAY_LIGHT = RGBColor(0xE5, 0xE5, 0xE5)
    PLACEHOLDER = RGBColor(0x3A, 0x3A, 0x3A)
    blank = prs.slide_layouts[6]

    for i, sd in enumerate(slides):
        s = prs.slides.add_slide(blank)
        t = sd.get("type", "content")

        if t == "title":
            _bg_fill(prs, s, BLACK)
            blk = s.shapes.add_shape(MSO_SHAPE.RECTANGLE, 0, 0, Inches(4.6), prs.slide_height)
            blk.fill.solid(); blk.fill.fore_color.rgb = YELLOW; _set_no_line(blk)
            _add_text(s, Inches(5.2), Inches(2.4), Inches(7.3), Inches(1.8), sd.get("title", ""), 36, WHITE, bold=True)
            _add_text(s, Inches(5.2), Inches(4.4), Inches(7), Inches(0.6), sd.get("subtitle", ""), 18, GRAY_LIGHT)

        elif t == "divider":
            _bg_fill(prs, s, YELLOW)
            _add_text(s, Inches(1), Inches(2.7), Inches(11.3), Inches(1.6), sd.get("title", ""), 40, BLACK, bold=True)
            _add_text(s, Inches(1), Inches(4.5), Inches(11), Inches(0.6), sd.get("subtitle", ""), 18, RGBColor(0x4A, 0x3A, 0x00))

        elif t == "cards_3":
            _bg_fill(prs, s, WHITE)
            _draw_cards_3(prs, s, sd, accent=YELLOW, card_bg=RGBColor(0xF7, 0xF7, 0xF7), title_color=BLACK, desc_color=RGBColor(0x5A, 0x5A, 0x5A), text_on_accent=BLACK)

        elif t == "stat_highlight":
            _bg_fill(prs, s, WHITE)
            _draw_stat_highlight(prs, s, sd, accent=YELLOW, title_color=BLACK, desc_color=RGBColor(0x5A, 0x5A, 0x5A))

        elif t == "split_image_text":
            _bg_fill(prs, s, WHITE)
            _draw_split_image_text(prs, s, sd, title_color=BLACK, text_color=RGBColor(0x5A, 0x5A, 0x5A), placeholder_color=PLACEHOLDER, image_right=(i % 2 == 0))

        else:
            _bg_fill(prs, s, WHITE)
            _add_image_or_fallback(s, sd.get("_image"), Inches(8.3), Inches(0), Inches(5.03), prs.slide_height, PLACEHOLDER)
            tag = s.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(0.9), Inches(0.8), Inches(1.7), Inches(0.5))
            tag.fill.solid(); tag.fill.fore_color.rgb = YELLOW; _set_no_line(tag)
            _add_text(s, Inches(0.9), Inches(1.7), Inches(7), Inches(1.0), sd.get("title", ""), 26, BLACK, bold=True)
            _add_bullets(s, Inches(0.9), Inches(2.9), Inches(7), Inches(4.0), sd.get("bullets", []), 14, BLACK)


_RENDERERS = {
    "blue": _render_blue,
    "dark": _render_dark,
    "warm": _render_warm,
    "yellow": _render_yellow,
}


# ─────────────────────────────────────────────────────────
# Asosiy funksiya
# ─────────────────────────────────────────────────────────

def build_slide_deck(topic: str, num_slides: int, template: str) -> bytes:
    """Mavzu, slaydlar soni va shablon nomiga qarab to'liq PPTX faylni yasaydi."""
    with_images, _ = build_slide_deck_dual(topic, num_slides, template)
    return with_images


def build_slide_deck_dual(topic: str, num_slides: int, template: str) -> tuple:
    """Mavzu bo'yicha matn va rasmlarni BIR MARTA generatsiya qilib (Gemini/Pexels
    so'rovlari qayta yuborilmaydi), ikkita PPTX variant qaytaradi:
    (rasmli_bayt, rasmsiz_bayt)."""
    num_slides = max(MIN_SLIDES, min(MAX_SLIDES, num_slides))
    template = template if template in _RENDERERS else "blue"

    slides_data = generate_outline(topic, num_slides)

    # Har bir "content" slayd uchun Pexels'dan rasm topamiz
    for sd in slides_data:
        if sd.get("type", "content") in ("content", "split_image_text") and sd.get("image_keyword"):
            sd["_image"] = fetch_pexels_image(sd["image_keyword"])

    # ── Rasmli variant ────────────────────────────────────
    prs_with = Presentation()
    prs_with.slide_width = Inches(13.333)
    prs_with.slide_height = Inches(7.5)
    _RENDERERS[template](prs_with, slides_data)
    buf_with = io.BytesIO()
    prs_with.save(buf_with)
    buf_with.seek(0)

    # ── Rasmsiz variant - xuddi shu matn, lekin rasm o'rniga
    # brendga mos rangli blok (shablon dizayni buzilmaydi) ──
    slides_no_image = [dict(sd) for sd in slides_data]
    for sd in slides_no_image:
        sd["_image"] = None
    prs_without = Presentation()
    prs_without.slide_width = Inches(13.333)
    prs_without.slide_height = Inches(7.5)
    _RENDERERS[template](prs_without, slides_no_image)
    buf_without = io.BytesIO()
    prs_without.save(buf_without)
    buf_without.seek(0)

    return buf_with.read(), buf_without.read()
