...
async def api_create_referat_order(request):
    ...
    muassasa = (payload.get("muassasa") or "").strip()
    fakultet = (payload.get("fakultet") or "").strip()
    guruh = (payload.get("guruh") or "").strip()
    try:
...
def _build_slide_result(order):
    """Slayd taqdimotini yasaydi (Gemini + Pexels + python-pptx).
    Tarmoq so'rovlari borligi uchun bu funksiya asyncio.to_thread orqali
    chaqiriladi."""
    data = slide_gen.build_slide_deck(order["topic"], order["num_slides"], order["template"])
