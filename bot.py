...
# ─────────────────────────────────────────────────────────
# P2P (qo'lda tasdiqlash) buyurtmalari - faqat "topup" (balans to'ldirish)
# turi uchun Supabase'ga ham yoziladi. Sabab: Render bepul tarifida server
# vaqti-vaqti bilan qayta ishga tushadi (deploy yoki uyqudan uyg'onish),
# va shu paytda RAM'dagi `orders` lug'ati tozalanadi - agar mijoz aynan
# shu oraliqda chek yuborgan bo'lsa, buyurtma iz-suzsiz yo'qolib, adminga
# hech qanday xabar kelmay qolishi mumkin edi. Endi shu holatda ham
# `p2p_orders` jadvalidan `pending`/`tekshirilmoqda` statusidagi
# buyurtmalarni SQL orqali topib olish mumkin. Boshqa xizmatlar
# (slayd/referat/obyektivka/test) buyurtmalariga bu TEGMAYDI - ular
# hali ham faqat RAM'da, chunki ular ichida generatsiya qilingan katta
# hujjat baytlari bor, bazaga yozish keraksiz og'irlik bo'lardi.
# ─────────────────────────────────────────────────────────

async def db_save_p2p_order(order_id: str, chat_id: int, amount: int, status: str):
    """Yangi P2P (balans to'ldirish) buyurtmasini Supabase'ga yozadi."""
    if not SUPABASE_URL or not SUPABASE_KEY:
        return
    try:
        session = await _get_supabase_session()
        url = f"{SUPABASE_URL}/rest/v1/p2p_orders"
        headers = _supabase_headers()
        headers["Prefer"] = "resolution=merge-duplicates"
        payload = {"order_id": order_id, "chat_id": chat_id, "amount": amount, "status": status}
        async with session.post(url, headers=headers, json=payload, timeout=aiohttp.ClientTimeout(total=10)) as resp:
            if resp.status not in (200, 201):
                logger.error(f"Supabase'ga p2p_order yozishda xato: {resp.status} {await resp.text()}")
    except Exception:
        logger.exception("Supabase'ga p2p_order yozishda xato")


async def db_update_p2p_order_status(order_id: str, status: str):
    """P2P buyurtma holatini yangilaydi (masalan 'tekshirilmoqda', 'tayyor')."""
    if not SUPABASE_URL or not SUPABASE_KEY:
        return
    try:
        session = await _get_supabase_session()
        ...
        async with session.patch(url, headers=_supabase_headers(), params=params, json=payload, timeout=aiohttp.ClientTimeout(total=10)) as resp:
            if resp.status not in (200, 204):
                logger.error(f"Supabase'da p2p_order holatini yangilashda xato: {resp.status} {await resp.text()}")
    except Exception:
        logger.exception("Supabase'da p2p_order holatini yangilashda xato")


async def db_fetch_pending_p2p_orders() -> list[dict]:
    """'To'lov kutilmoqda' yoki 'tekshirilmoqda' holatidagi barcha P2P
    buyurtmalarni qaytaradi - agar bot qayta tushib RAM tozalansa ham,
    admin shu ro'yxatni SQL orqali (yoki kelajakda /pending_orders
    buyrug'i orqali) ko'rib, qo'lda tekshira oladi."""
    if not SUPABASE_URL or not SUPABASE_KEY:
        return []
    try:
        session = await _get_supabase_session()
        url = f"{SUPABASE_URL}/rest/v1/p2p_orders"
        params = {"status": "in.(to'lov_kutilmoqda,tekshirilmoqda)", "order": "created_at.desc"}
        async with session.get(url, headers=_supabase_headers(), params=params, timeout=aiohttp.ClientTimeout(total=10)) as resp:
            if resp.status != 200:
                logger.error(f"Supabase'dan p2p_orders o'qishda xato: {resp.status} {await resp.text()}")
                return []
            return await resp.json()
    except Exception:
        logger.exception("Supabase'dan p2p_orders o'qishda xato")
        return []
...
# ─────────────────────────────────────────────────────────
# Checkout.uz orqali avtomatik to'lov (Click/Payme/karta).
# Topchiq.uz loyihasidagi tajriba asosida: webhook'ga hech qachon
# ishonilmaydi, har doim checkout_uz.get_payment_status() orqali
# HAQIQAT tekshiriladi, va balans faqat bir marta (atomik) qo'shiladi.
# ─────────────────────────────────────────────────────────

async def db_insert_checkout_payment(payment_id: str, uuid_val: str | None, chat_id: int, amount: int) -> bool:
    if not SUPABASE_URL or not SUPABASE_KEY:
        return False
    try:
        session = await _get_supabase_session()
        url = f"{SUPABASE_URL}/rest/v1/checkout_payments"
        payload = {
        ...
        async with session.post(url, headers=_supabase_headers(), json=payload, timeout=aiohttp.ClientTimeout(total=10)) as resp:
            if resp.status in (200, 201):
                return True
            logger.error(f"checkout_payments yozishda xato: {resp.status} {await resp.text()}")
            return False
    except Exception:
        logger.exception("checkout_payments yozishda xato")
        return False


async def db_fetch_checkout_payment(payment_id: str) -> dict | None:
    if not SUPABASE_URL or not SUPABASE_KEY:
        return None
    try:
        session = await _get_supabase_session()
        url = f"{SUPABASE_URL}/rest/v1/checkout_payments"
        params = {"payment_id": f"eq.{payment_id}", "select": "payment_id,chat_id,amount,status"}
        async with session.get(url, headers=_supabase_headers(), params=params, timeout=aiohttp.ClientTimeout(total=10)) as resp:
            if resp.status != 200:
                return None
            rows = await resp.json()
        ...
        logger.exception("checkout_payments o'qishda xato")
        return None


async def db_fetch_pending_checkout_ids(chat_id: int) -> list[str]:
    """Shu foydalanuvchining hali 'pending' holatidagi barcha to'lovlari.
    Bu - Mini App JS xotirasiga (state) bog'liq bo'lmasdan, oyna
    yopilib qayta ochilganda ham to'lovni topib, tekshirib olish uchun."""
    if not SUPABASE_URL or not SUPABASE_KEY:
        return []
    try:
        session = await _get_supabase_session()
        ...
        async with session.get(url, headers=_supabase_headers(), params=params, timeout=aiohttp.ClientTimeout(total=10)) as resp:
            if resp.status != 200:
                return []
            rows = await resp.json()
            return [r["payment_id"] for r in rows if "payment_id" in r]
    except Exception:
        logger.exception("Pending checkout to'lovlarini o'qishda xato")
        return []


async def db_mark_checkout_paid(payment_id: str) -> bool:
    """Optimistik concurrency: status hali 'pending' bo'lsagina 'paid'ga
    o'zgartiradi. Webhook va 'tekshirish' tugmasi bir vaqtda kelsa ham,
    FAQAT BIRTASI muvaffaqiyatli bo'ladi - shu orqali balans ikki marta
    qo'shilib ketishining oldi olinadi (promo_codes'dagi used_count
    mantig'i bilan bir xil g'oya)."""
    if not SUPABASE_URL or not SUPABASE_KEY:
        return False
    try:
        session = await _get_supabase_session()
        ...
        payload = {"status": "paid", "paid_at": _now_iso()}
        async with session.patch(url, headers=headers, params=params, json=payload, timeout=aiohttp.ClientTimeout(total=10)) as resp:
            if resp.status not in (200, 206):
                logger.error(f"checkout_payments yangilashda xato: {resp.status} {await resp.text()}")
                return False
            rows = await resp.json()
            return len(rows) > 0
    except Exception:
        logger.exception("checkout_payments yangilashda xato")
        return False


async def confirm_checkout_payment(payment_id: str, bot: Bot | None = None) -> dict:
    """Checkout.uz to'lovini tasdiqlaydi. Bu funksiya IKKI YO'LDAN ham
    (webhook va Mini App'dagi 'Tekshirish' tugmasi) xuddi shu tarzda
    chaqiriladi - qaysi biri birinchi kelsa, o'sha ishlaydi, ikkinchisi
    hech narsa qilmaydi (allaqachon 'paid' bo'lgani uchun)."""
    row = await db_fetch_checkout_payment(payment_id)
    if not row:
        return {"ok": False, "reason": "topilmadi"}

    if row.get("status") == "paid":
        return {"ok": True, "reason": "allaqachon_tasdiqlangan"}

    try:
        truth = await checkout_uz.get_payment_status(payment_id=payment_id)
    except checkout_uz.CheckoutError:
        logger.exception(f"Checkout: {payment_id} holatini so'rashda xato")
        return {"ok": False, "reason": "tekshirib_bolmadi"}

    if not truth or truth.get("status") != "paid":
        return {"ok": False, "reason": "hali_tolanmagan"}

    expected_amount = int(row.get("amount") or 0)
    paid_amount = int(truth.get("amount") or 0)
    if paid_amount < expected_amount:
        logger.error(
            f"Checkout: {payment_id} summasi mos emas! kutilgan={expected_amount}, kelgan={paid_amount}"
        )
        return {"ok": False, "reason": "summa_mos_emas"}

    if not await db_mark_checkout_paid(payment_id):
        # Boshqa yo'l (webhook yoki tugma) allaqachon tasdiqlab ulgurgan
        return {"ok": True, "reason": "allaqachon_tasdiqlangan"}

    chat_id = row.get("chat_id")
    await add_balance(chat_id, expected_amount)
    new_balance = await get_balance(chat_id)
    ...
    if bot:
        try:
            ...
            logger.exception("Checkout: mijozga xabar yuborishda xato")

    return {"ok": True, "reason": "tasdiqlandi", "amount": expected_amount, "balance": new_balance}
...
@router.message(F.text == "/pending_orders")
async def cmd_pending_orders(message: Message):
    """Faqat admin uchun: Supabase'dagi 'to'lov_kutilmoqda'/'tekshirilmoqda'
    holatidagi P2P (balans to'ldirish) buyurtmalari - bot qayta tushib
    RAM tozalangan taqdirda ham, bu yerdan qaysi mijoz qancha to'lov
    qilganini ko'rish mumkin."""
    if message.from_user.id != ADMIN_CHAT_ID:
        return

    rows = await db_fetch_pending_p2p_orders()
    if not rows:
        await message.answer("✅ Kutilayotgan P2P buyurtmalar yo'q.")
        return

    lines = ["⏳ <b>Kutilayotgan P2P buyurtmalar</b>\n"]
    for r in rows[:30]:
        lines.append(
            f"• <code>{r.get('order_id')}</code> — chat_id: <code>{r.get('chat_id')}</code>, "
            f"{r.get('amount')} so'm, holat: {r.get('status')}, {r.get('created_at', '')[:16]}"
        )
...
async def api_create_order(request):
    ...
    asyncio.create_task(db_touch_user(user.get("id"), user.get("username")))
...
async def api_create_topup_order(request):
    ...
    asyncio.create_task(db_save_p2p_order(order_id, chat_id, amount, "to'lov_kutilmoqda"))
...
async def api_create_checkout_payment(request):
    """Mini App'dan - Click/Payme orqali AVTOMATIK to'lov havolasini
    yaratadi (Checkout.uz). P2P (qo'lda tasdiqlash) usuliga alternativ -
    uni almashtirmaydi, ikkalasi ham parallel ishlaydi."""
    try:
        payload = await request.json()
    except Exception:
        return web.json_response({"ok": False, "error": "Noto'g'ri so'rov"}, status=400)
    ...
    base_url = WEBAPP_URL.split("/webapp")[0].rstrip("/") if "/webapp" in WEBAPP_URL else WEBAPP_URL.rstrip("/")
    webhook_url = f"{base_url}/api/checkout_webhook"

    try:
        payment = await checkout_uz.create_payment(
            amount=amount,
            description=f"Madesy balans to'ldirish - {chat_id}",
            webhook_url=webhook_url,
        )
    except checkout_uz.CheckoutError as e:
        logger.exception("Checkout.uz to'lov yaratishda xato")
        return web.json_response({"ok": False, "error": str(e)}, status=502)

    saved = await db_insert_checkout_payment(payment["id"], payment.get("uuid"), chat_id, amount)
    if not saved:
        return web.json_response(
            {"ok": False, "error": "To'lovni saqlashda xato, birozdan keyin urinib ko'ring."},
            status=500,
        )
...
async def api_check_pending_checkouts(request):
    """Mini App ochilganda / qayta ko'rinishga kelganda AVTOMATIK
    chaqiriladi (tugma bosilishini kutmasdan). Shu foydalanuvchining
    hali 'pending' turgan barcha to'lovlarini Checkout.uz'dan tekshirib,
    to'langanlarini avtomatik balansga qo'shadi."""
    try:
        payload = await request.json()
    except Exception:
        return web.json_response({"ok": False, "error": "Noto'g'ri so'rov"}, status=400)
    ...
    chat_id = user.get("id")
    pending_ids = await db_fetch_pending_checkout_ids(chat_id)

    confirmed = 0
    if pending_ids:
        bot: Bot = request.app["bot"]
        for pid in pending_ids:
            result = await confirm_checkout_payment(pid, bot=bot)
            if result.get("reason") == "tasdiqlandi":
                confirmed += 1

    balance = await get_balance(chat_id)
    return web.json_response({"ok": True, "confirmed": confirmed, "balance": balance})


async def api_check_checkout_payment(request):
    """Mini App'dagi 'To'ladim, tekshirish' tugmasi bosilganda chaqiriladi.
    Checkout.uz'dan HAQIQATNI so'raydi (webhook'ga ishonmaydi) va agar
    to'langan bo'lsa, balansni oshiradi."""
    try:
        payload = await request.json()
    except Exception:
        return web.json_response({"ok": False, "error": "Noto'g'ri so'rov"}, status=400)
    ...
    if not payment_id:
        return web.json_response({"ok": False, "error": "payment_id kerak"}, status=400)

    row = await db_fetch_checkout_payment(payment_id)
    if not row or int(row.get("chat_id") or 0) != chat_id:
        return web.json_response({"ok": False, "error": "Bunday to'lov topilmadi"}, status=404)

    bot: Bot = request.app["bot"]
    result = await confirm_checkout_payment(payment_id, bot=bot)

    if result.get("reason") in ("tasdiqlandi", "allaqachon_tasdiqlangan"):
    ...
    reason_messages = {
        "hali_tolanmagan": "To'lov hali amalga oshirilmagan. Agar to'lagan bo'lsangiz, biroz kutib qayta urinib ko'ring.",
        "tekshirib_bolmadi": "To'lov holatini tekshirib bo'lmadi, birozdan keyin qayta urinib ko'ring.",
        "summa_mos_emas": "To'lov summasida nomuvofiqlik aniqlandi. Admin bilan bog'laning.",
        "topilmadi": "Bunday to'lov topilmadi.",
    }
    return web.json_response(
        {"ok": False, "error": reason_messages.get(result.get("reason"), "To'lov hali tasdiqlanmadi.")},
        status=400,
    )


async def api_checkout_webhook(request):
    """Checkout.uz'ning o'zi chaqiradigan webhook. DIQQAT: bu yerda
    kelgan ma'lumotga ISHONILMAYDI - faqat 'signal' sifatida qabul
    qilinadi, so'ng checkout_uz.get_payment_status() orqali HAQIQAT
    tekshiriladi (confirm_checkout_payment ichida)."""
    try:
        payload = await request.json()
    except Exception:
        payload = {}
    ...
    payment_id = (
        ...
        or (payload.get("payment") or {}).get("_id")
        or (payload.get("data") or {}).get("id")
    )
    if not payment_id:
        logger.warning(f"Checkout webhook: id topilmadi, payload={payload}")
        return web.json_response({"ok": True})  # 200 qaytaramiz, aks holda qayta-qayta yuborishi mumkin

    bot: Bot = request.app["bot"]
    asyncio.create_task(confirm_checkout_payment(str(payment_id), bot=bot))
...
async def _send_photo3x4_result(bot: Bot, chat_id: int, single_bytes: bytes):
    """AI 3x4 natijasini (bitta rasm + 8 nusxali varaq) botga yuboradi."""
    sheet_bytes = await asyncio.to_thread(photo_ai_3x4.build_sheet_8x, single_bytes)

    await bot.send_document(
        chat_id,
        BufferedInputFile(single_bytes, filename="3x4_rasm.jpg"),
        caption="✅ 3×4 rasmingiz tayyor! (bitta nusxa)",
    )
    await bot.send_document(
        chat_id,
        BufferedInputFile(sheet_bytes, filename="3x4_varaq_8ta.jpg"),
        caption="✅ 15x10 sm varaq - 8 nusxa, kesish chiziqlari bilan.",
    )


async def api_create_photo3x4_order(request):
    """AI 3x4 rasm: to'lov yechiladi, OpenAI GPT Image 2 orqali rasmiy
    portret generatsiya qilinadi va natija (bitta rasm + 8 nusxali varaq)
    botga yuboriladi. Qayta ishlash imkoniyati yo'q - bitta to'lov,
    bitta natija."""
    try:
        payload = await request.json()
    except Exception:
    ...
    price = photo_ai_3x4.PRICE_3X4
    balance = await get_balance(chat_id)
    if balance < price:
        return web.json_response(
            {"ok": False, "error": f"Balansingiz yetarli emas. Kerak: {price} so'm, mavjud: {balance} so'm."},
            status=400,
        )

    deducted = await deduct_balance(chat_id, price)
    if not deducted:
        return web.json_response({"ok": False, "error": "Balansdan yechishda xatolik"}, status=400)

    bot: Bot = request.app["bot"]
    try:
        async with _photo_ai_3x4_semaphore:
            result_bytes = await asyncio.to_thread(
                photo_ai_3x4.process_photo_to_3x4, photo_bytes
            )
    except Exception as e:
        logger.error(f"AI 3x4 rasm xatosi: {e}")
        # Xato yuz bersa, yechilgan pulni qaytaramiz - foydalanuvchi aybi emas
        await add_balance(chat_id, price)
        return web.json_response(
            {"ok": False, "error": "Rasmni qayta ishlashda xatolik yuz berdi. Balansingiz qaytarildi."},
            status=500,
        )

    await _send_photo3x4_result(bot, chat_id, result_bytes)
...
async def api_submit_receipt(request):
    ...
    if order.get("service") == "topup":
        asyncio.create_task(db_update_p2p_order_status(order_id, "tekshirilmoqda"))
...
# AI-kuchaytirilgan 3x4 (OpenAI GPT Image 2) - bu tashqi API so'rovi,
# lokal xotiraga unchalik og'irlik solmaydi, lekin gpt-image-2 modelining
# Tier 1'da "5 images per minute" chegarasi bor - bu chegaraga tegib
# ketmaslik uchun bir vaqtning o'zida ko'pi bilan 4 ta so'rovga ruxsat
# beramiz (xavfsiz zaxira bilan, 5'dan bittaga kam).
_photo_ai_3x4_semaphore = asyncio.Semaphore(4)
...
async def order_approve(callback: CallbackQuery, bot: Bot):
    ...
    if service == "topup":
        ...
        order["status"] = "tayyor"
        asyncio.create_task(db_update_p2p_order_status(order_id, "tayyor"))
        try:
            await bot.send_message(
                order["chat_id"],
...
async def order_reject(callback: CallbackQuery, bot: Bot):
    ...
    if order and order.get("status") not in ("rad_etildi", "ishlanmoqda", "tayyor"):
        ...
        if order.get("service") == "topup":
            asyncio.create_task(db_update_p2p_order_status(order_id, "rad_etildi"))
        try:
            await bot.send_message(
                order["chat_id"],
...
async def start_web_server(bot: Bot):
    ...
    app.router.add_post("/api/create_checkout_payment", api_create_checkout_payment)
    app.router.add_post("/api/check_checkout_payment", api_check_checkout_payment)
    app.router.add_post("/api/check_pending_checkouts", api_check_pending_checkouts)
    app.router.add_post("/api/checkout_webhook", api_checkout_webhook)
