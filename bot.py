...
def add_balance(chat_id: int, amount: int):
    user_balances[chat_id] = user_balances.get(chat_id, 0) + amount


def deduct_balance(chat_id: int, amount: int) -> bool:
...
async def api_create_order(request):
    ...
    try:
        ...
        add_balance(chat_id, cost)  # xato - balansni qaytaramiz
    ...
    return web.json_response({"ok": True, "balance": get_balance(chat_id)})
...
async def api_get_balance(request):
    ...
    return web.json_response({"ok": True, "balance": get_balance(user.get("id"))})
...
async def api_submit_receipt(request):
    ...
    if service == "topup":
        details = (
            ...
            f"Joriy balans: {get_balance(order['chat_id'])} so'm"
        )
    else:
...
async def order_approve(callback: CallbackQuery, bot: Bot):
    ...
    if service == "topup":
        add_balance(order["chat_id"], order["amount"])
        new_balance = get_balance(order["chat_id"])
        order["status"] = "tayyor"
        try:
