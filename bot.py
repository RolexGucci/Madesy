...
async def _send_photo3x4_result(bot: Bot, chat_id: int, single_bytes: bytes, attempts_left: int):
    ...
    await bot.send_document(
        ...
        caption=(
            "✅ 10x15 sm varaq - 8 nusxa, kesish chiziqlari bilan.\n\n"
            + (
                f"Natija yoqmasa, Mini App'da \"Qayta ishlash\" tugmasini bosing "
                f"({attempts_left} marta bepul qoldi)."
                if attempts_left > 0
                else "Bepul qayta ishlash imkoniyati tugadi."
            )
        ),
    )
...
async def api_create_photo3x4_order(request):
    """AI 3x4 rasm - 1-urinish: to'lov yechiladi, Gemini orqali rasmiy
    portret generatsiya qilinadi va natija (bitta rasm + 8 nusxali varaq)
    botga yuboriladi. Original selfie xotirada saqlanadi - shu orqali
    keyingi bepul qayta urinishlar (retry) yangi rasm yuklamasdan amalga
    oshadi."""
    try:
        payload = await request.json()
    except Exception:
    ...
    try:
        async with _photo_ai_3x4_semaphore:
            result_bytes = await asyncio.to_thread(
                photo_ai_3x4.process_photo_to_3x4, photo_bytes, 0
            )
    except Exception as e:
    ...
    photo_ai_3x4_orders[chat_id] = {"original_bytes": photo_bytes, "attempts_used": 1}
    attempts_left = PHOTO_AI_3X4_MAX_ATTEMPTS - 1

    await _send_photo3x4_result(bot, chat_id, result_bytes, attempts_left)

    new_balance = await get_balance(chat_id)
    return web.json_response({"ok": True, "balance": new_balance, "attempts_left": attempts_left})


async def api_retry_photo3x4(request):
    """AI 3x4 rasm - qayta ishlash: bepul, lekin FAQAT bir xil, avval
    yuklangan original selfie bilan (yangi rasm qabul qilinmaydi). Jami
    PHOTO_AI_3X4_MAX_ATTEMPTS martagacha (1 to'lov + 3 bepul) ishlatilishi
    mumkin."""
    try:
        payload = await request.json()
    except Exception:
    ...
    order = photo_ai_3x4_orders.get(chat_id)
    if not order:
        return web.json_response(
            {"ok": False, "error": "Avval yangi 3x4 buyurtma bering - qayta ishlash uchun asos topilmadi."},
            status=400,
        )

    if order["attempts_used"] >= PHOTO_AI_3X4_MAX_ATTEMPTS:
        return web.json_response(
            {"ok": False, "error": "Bepul qayta ishlash imkoniyati tugadi. Yangi buyurtma uchun qayta to'lov talab qilinadi."},
            status=400,
        )

    bot: Bot = request.app["bot"]
    attempt_index = order["attempts_used"]  # 1, 2 yoki 3
    try:
        async with _photo_ai_3x4_semaphore:
            result_bytes = await asyncio.to_thread(
                photo_ai_3x4.process_photo_to_3x4, order["original_bytes"], attempt_index
            )
    except Exception as e:
        logger.error(f"AI 3x4 qayta ishlash xatosi: {e}")
        return web.json_response(
            {"ok": False, "error": "Qayta ishlashda xatolik yuz berdi. Birozdan so'ng qayta urinib ko'ring."},
            status=500,
        )

    order["attempts_used"] += 1
    attempts_left = PHOTO_AI_3X4_MAX_ATTEMPTS - order["attempts_used"]

    await _send_photo3x4_result(bot, chat_id, result_bytes, attempts_left)

    return web.json_response({"ok": True, "attempts_left": attempts_left})
...
# Har bir foydalanuvchining joriy AI 3x4 buyurtmasini saqlaydi - shu orqali
# "qayta ishlash" so'ralganda ORIGINAL selfie qayta yuklanmasdan, xotiradan
# olinadi. Bitta buyurtma tugagach (muvaffaqiyatli yakunlangach yoki yangi
# buyurtma boshlanganda) yozuv almashtiriladi/o'chiriladi.
# Struktura: {chat_id: {"original_bytes": bytes, "attempts_used": int}}
photo_ai_3x4_orders: dict[int, dict] = {}
PHOTO_AI_3X4_MAX_ATTEMPTS = 1 + photo_ai_3x4.MAX_FREE_RETRIES  # 1 to'lov + 3 bepul qayta urinish
...
async def start_web_server(bot: Bot):
    ...
    app.router.add_post("/api/retry_photo3x4", api_retry_photo3x4)
