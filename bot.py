...
async def cmd_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        "👋 Assalom Aleykum ... Xush kelibsiz 🫂\n\n"
        "✨ <b>Madesy</b> - Made all Easy ✨\n\n"
        "💠   Oson   💠   Arzon   💠   Tez",
...
async def api_download(request):
    """4-qadam: admin tasdiqlagach, tayyor hujjatni Mini App shu orqali yuklab oladi."""
    order_id = request.match_info.get("order_id", "")
    order = orders.get(order_id)
    if not order or order["status"] != "tayyor" or not order["result_bytes"]:
        return web.json_response({"ok": False, "error": "Hujjat hali tayyor emas"}, status=404)

    content_type = (
        "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
        if order["fmt"] != "pdf" else "application/pdf"
    )
    return web.Response(
        body=order["result_bytes"],
        content_type=content_type,
        headers={"Content-Disposition": f'attachment; filename="{order["result_filename"]}"'},
    )
...
async def order_approve(callback: CallbackQuery):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            order["result_bytes"] = buf.read()
            order["result_filename"] = filename
            order["status"] = "tayyor"
        except Exception:
            logger.exception("Tasdiqlangandan keyin hujjat yasashda xato")
            order["status"] = "xato"
            await callback.message.edit_caption(
                caption=callback.message.caption + "\n\n❌ Hujjat yasashda xato yuz berdi.",
            )
            return

    await callback.message.edit_caption(
        caption=callback.message.caption + "\n\n✅ Tasdiqlandi, mijoz Mini App orqali yuklab olishi mumkin.",
    )
...
async def start_web_server(bot: Bot):
    ...
    app.router.add_get("/api/download/{order_id}", api_download)
