...
from photo_3x4 import process_3x4_photo
...
async def menu_photo(callback: CallbackQuery, state: FSMContext):
    await state.set_state(PhotoForm.waiting_gender)
    await callback.message.edit_text(
        "📸 <b>3x4 rasm tayyorlash</b>\n\n"
        "Avval jinsingizni tanlang (kostyum shakli shunga qarab tanlanadi):",
...
@router.callback_query(PhotoForm.waiting_gender, F.data.in_({"gender_male", "gender_female"}))
async def photo_gender_chosen(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        "💡 <i>Yaxshi natija uchun: yorug' joyda, oldinga qarab, yuz aniq ko'rinadigan rasm yuboring.</i>",
        parse_mode="HTML",
    )
    await callback.answer()
...
async def photo_received(message: Message, state: FSMContext, bot: Bot):
    processing_msg = await message.answer("⚙️ Rasm tahrirlanmoqda, biroz kuting (5-10 soniya)...")
    ...
    try:
        ...
        result_bytes = await asyncio.to_thread(process_3x4_photo, input_bytes, gender)

        await message.answer_document(
            BufferedInputFile(result_bytes, filename="3x4_rasm.jpg"),
            caption="✅ 3x4 rasmingiz tayyor!",
        )
    except Exception as e:
        logger.exception("3x4 rasm ishlov berishda xato")
        await message.answer(
            "❌ Rasmni tahrirlashda xatolik yuz berdi. Boshqa rasm bilan qayta urinib ko'ring.\n\n"
            f"<i>Texnik tafsilot: {str(e)[:200]}</i>",
            parse_mode="HTML",
        )
    finally:
        try:
    ...
    await message.answer("Yana nima kerak?", reply_markup=kb_home())
    await state.clear()
...
async def api_photo(request):
    """Mini App'dagi mustaqil '3x4 rasm' xizmati uchun endpoint."""
    bot: Bot = request.app["bot"]
    try:
        payload = await request.json()
    except Exception as e:
        logger.warning(f"api_photo: so'rov tanasini o'qishda xato: {e}")
        return web.json_response({"ok": False, "error": "Noto'g'ri so'rov"}, status=400)

    init_data = payload.get("initData", "")
    user = validate_init_data(init_data, BOT_TOKEN)
    if not user:
        logger.warning("api_photo: autentifikatsiya muvaffaqiyatsiz")
        return web.json_response({"ok": False, "error": "Autentifikatsiya xatosi"}, status=401)

    chat_id = user.get("id")
    photo_b64 = payload.get("photo_base64")
    gender = payload.get("gender")  # "male" | "female" | None
    logger.info(f"api_photo: qabul qilingan gender = {gender!r}")
    if not photo_b64:
        logger.warning("api_photo: photo_base64 yuborilmagan")
        return web.json_response({"ok": False, "error": "Rasm topilmadi"}, status=400)
    ...
    try:
        ...
        logger.exception("Mini App orqali 3x4 rasm ishlov berishda xato")
        return web.json_response({"ok": False, "error": str(e)}, status=500)
