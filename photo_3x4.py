"""
[DEPRECATED - endi bot.py'da ishlatilmaydi, o'rniga photo_ai_3x4.py
ishlatiladi. Bu fayl faqat tarixiy/zaxira sifatida saqlanmoqda, xohlasangiz
GitHub'dan butunlay o'chirib tashlashingiz mumkin.]
...
def _sharpen_mask(mask: Image.Image, low: int = 110, high: int = 150) -> Image.Image:
    """Maskaning "xira" (yarim-shaffof) chekkalarini keskinlashtiradi -
    aks holda past ishonchli piksellar orqali orqa fon rangi (masalan
    qizil devor) soch/quloq atrofida "sizib chiqib" ko'rinadi."""
    lut = []
    for p in range(256):
        if p <= low:
            lut.append(0)
            ...
            lut.append(int((p - low) / (high - low) * 255))
