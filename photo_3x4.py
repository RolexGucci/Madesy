...
def _load_suit_template(gender: str) -> Image.Image:
    """Kostyum shablonini (PNG, fon shaffof) diskdan yuklaydi va keshda saqlaydi."""
    key = "male" if gender == "male" else "female"
    if key not in _suit_cache:
        path = os.path.join(ASSETS_DIR, f"suit_{key}.png")
        _suit_cache[key] = Image.open(path).convert("RGBA")
...
def _apply_suit_overlay(final_rgb: Image.Image, gender: str) -> Image.Image:
    ...
    if new_h < needed_h:
        # Shablon canvas tagigacha yetmasa, oxirgi qatorni pastga cho'zib to'ldiramiz
        last_row = resized.crop((0, new_h - 1, width, new_h))
        extra = last_row.resize((width, needed_h - new_h))
        extended = Image.new("RGBA", (width, needed_h), (0, 0, 0, 0))
        extended.paste(resized, (0, 0))
        extended.paste(extra, (0, new_h))
        resized = extended
    elif new_h > needed_h:
        resized = resized.crop((0, 0, width, needed_h))
