...
Fon olib tashlash uchun 'rembg' kutubxonasidan foydalanadi - bu server ichida
ishlaydi, hech qanday tashqi API, kalit yoki oylik limit kerak emas.

Qo'shimcha (hammasi bepul, mahalliy):
...
def _get_session():
    """Model faqat bir marta yuklanadi (birinchi chaqiruvda), keyin xotirada saqlanadi.
    u2netp - kichik versiya (~4.7MB), Render bepul reja (512MB RAM) uchun mos.
    """
    global _session
    if _session is None:
...
def process_3x4_photo(input_bytes: bytes, add_suit: bool = True) -> bytes:
    ...
    # rembg katta rasmlarda sekin ishlaydi - hisoblash tezligini oshirish uchun cheklaymiz
    input_img.thumbnail((1200, 1200), Image.LANCZOS)

    removed = remove(input_img, session=session)  # RGBA, fon shaffof
