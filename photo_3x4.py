...
def _apply_suit_overlay(final_rgb: Image.Image, gender: str) -> Image.Image:
    ...
    neck_y = int(height * 0.58)
    needed_h = height - neck_y

    if new_h >= needed_h:
        # Shablonning PASTKI qismini olamiz - kiyim aynan shu yerda,
        # tepasida esa bo'sh/fon joy bor edi
        resized = resized.crop((0, new_h - needed_h, width, new_h))
    else:
        # Shablon juda qisqa bo'lsa, tepasidan bir xil rangda cho'zib to'ldiramiz
        first_row = resized.crop((0, 0, width, 1))
        extra = first_row.resize((width, needed_h - new_h))
        extended = Image.new("RGBA", (width, needed_h), (0, 0, 0, 0))
        extended.paste(extra, (0, 0))
        extended.paste(resized, (0, needed_h - new_h))
        resized = extended

    final_rgba = final_rgb.convert("RGBA")
    final_rgba.alpha_composite(resized, (0, neck_y))
