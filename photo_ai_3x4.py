...
Muhim parametr: input_fidelity="high" - bu model original rasmdagi
yuz xususiyatlariga imkon qadar yaqin qolishga urinishini bildiradi
(rasmiy OpenAI parametri, aynan "identity preservation" uchun).
...
Oqim:
  ...
  2. GPT Image /v1/images/edits so'rovi yuboriladi: fonni oq qil,
     ...
     xos qiyofasini o'zgartirma (input_fidelity=high bilan).
...
Narx va qayta urinish siyosati (bot.py'da amalga oshiriladi, bu yerda
faqat texnik generatsiya funksiyasi bor):
  - 1-urinish - to'lov bilan (5000 so'm)
  - 2, 3, 4-urinishlar - bepul, lekin FAQAT bir xil original selfie bilan
    (foydalanuvchi yangi rasm yuklay olmaydi, faqat "qayta ishlash"ni bosadi)
  - Har bir qayta urinishda natija bir xil chiqib qolmasligi uchun
    promptga ozgina variatsiya qo'shiladi (pastga qarang: _VARIATION_HINTS)
"""
import base64
import io
import logging
...
PRICE_3X4 = 5000
MAX_FREE_RETRIES = 3  # 1-urinishdan tashqari yana shuncha bepul urinish (jami 4 ta)

# 10x15 sm, 300 DPI'da piksel o'lchami
SHEET_WIDTH = 1181   # 10 sm
SHEET_HEIGHT = 1772  # 15 sm

_BASE_PROMPT = """Rasmiy 3×4 pasport uslubidagi hujjat rasmi yasang.
...
_VARIATION_HINTS = [
    "",  # 1-urinish - qo'shimcha ko'rsatmasiz
    "Bu qayta ishlash: yuzni oldingi urinishga qaraganda ANIQROQ original "
    "rasmga o'xshat - agar yuz xususiyatlari birinchi urinishda biroz "
    "o'zgargan bo'lsa, bu safar originalga yanada yaqinroq bo'lsin. "
    "Yoritishni biroz yumshoqroq va tekisroq qil.",
    "Bu qayta ishlash: yuz proporsiyalarini (ko'z oralig'i, burun, lab "
    "shakli) original selfiedagidek ANIQ saqla - hech qanday "
    "'go'zallashtirish' yoki 'silliqlashtirish' qilma. Kostyum va "
    "ko'ylakni yanada tekis moslashtir.",
    "Bu qayta ishlash: eng muhimi - yuz original rasmdagi odamga 100% "
    "o'xshasin, hatto agar bu fon/kiyim sifatini biroz pasaytirishga "
    "to'g'ri kelsa ham. Teri tonini tabiiy va yumshoq qil, haddan "
    "tashqari silliqlashtirilgan ko'rinishdan qoch.",
]


def _build_prompt(attempt_index: int) -> str:
    """attempt_index: 0 = birinchi urinish, 1-3 = qayta urinishlar."""
    hint = _VARIATION_HINTS[min(attempt_index, len(_VARIATION_HINTS) - 1)]
    return _BASE_PROMPT.format(variation_hint=hint)
...
def generate_formal_portrait(image_bytes: bytes, attempt_index: int = 0) -> bytes:
    """OpenAI GPT Image 2 (/v1/images/edits) orqali selfie'ni rasmiy
    portretga aylantiradi.

    image_bytes: xom selfie baytlari.
    attempt_index: 0 = birinchi urinish, 1/2/3 = qayta urinishlar (promptga
      ozgina variatsiya qo'shiladi, natija farqliroq chiqishi uchun).
    ...
    prompt = _build_prompt(attempt_index)
    ...
    data = {
        ...
        "background": "opaque",
        "input_fidelity": "high",  # yuz xususiyatlarini imkon qadar saqlash
        "n": 1,
    }
...
def process_photo_to_3x4(image_bytes: bytes, attempt_index: int = 0) -> bytes:
    """To'liq oqim: selfie -> OpenAI GPT Image 2 qayta ishlash -> 3x4
    o'lchamga moslashtirish. Qaytaradi: bitta 3x4 JPEG bayt."""
    ai_result = generate_formal_portrait(image_bytes, attempt_index=attempt_index)
...
def build_sheet_8x(single_3x4_bytes: bytes) -> bytes:
    """Bitta 3x4 rasmdan 10x15 sm varaqda 8 nusxa (2 ustun x 4 qator)
    joylashtirilgan, kesish chiziqlari bilan JPEG tayyorlaydi - chop
