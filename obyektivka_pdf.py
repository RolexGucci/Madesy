...
def build_obyektivka_pdf(data: dict, photo_path: str = None) -> io.BytesIO:
    ...
    if photo_path and os.path.exists(photo_path):
        try:
            img = Image(photo_path, width=2.5 * cm, height=3.3 * cm)
            photo_table = Table([[img]], colWidths=[2.7 * cm], rowHeights=[3.5 * cm])
            photo_table.setStyle(TableStyle([
                ("BOX", (0, 0), (-1, -1), 1, colors.black),
                ("ALIGN", (0, 0), (-1, -1), "CENTER"),
                ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
            ]))
            header_table = Table([[left_content, photo_table]], colWidths=[13 * cm, 3 * cm])
        except Exception:
